...
balance:
    ...
    label skew:     1 Quantity:  each party owns data samples of a fixed number of labels (chaque client n'a qu'une seule classe)
                    2 Dirichlet: each party is allocated a proportion of the samples of each label according to Dirichlet distribution.
                    3 Shard: each party is allocated the same numbers of shards (nbr de sample) that is sorted by the labels of the data
-----------------------------------------------------------------------------------
...
# the core_cifar109.py at the path of these datasets for details).
class BasicTaskGen:
    _TYPE_DIST = {
        0: 'iid',
...
#      datasets in torchvision and torchspeech. Examples can be found in mnist\core_cifar109.py, cifar\core_cifar109.py.
#
#   2) Save the partitioned data itself into .json file. Then read the data. The advantage of this way is the flexibility.
#      Examples can be found in emnist\core_cifar109.py,, synthetic\core_cifar109.py quad2d\core_cifar109.py.
